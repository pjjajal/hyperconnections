...
from hyperconnections.ops.numbers import (
    _a11, _a21, _a31,
    _b11, _b21, _b31, _b61,
...
### SANDBOX IDEA (launch/dispatch overhead): the phi_1 Frechet backward (Part 2)
### rebuilds a [B,3N,3N] augmented M every call — a ~38MB torch.zeros + a fresh
### torch.eye + its scatter, on top of the data-block writes.  Two prior
### iterations showed the kernel itself is near-optimal, so the remaining cost
### is this Python-side assembly.  Reuse a pre-zeroed buffer keyed by
### (B,N,device): the zero blocks and the identity block (rows N:2N, cols
### 2N:3N) are invariant across calls and written once on allocation; only the
### three data blocks (A^T, G_psi, A^T) are rewritten per backward.  Steady-state
### batch sizes hit the cache every call, removing the zeroing + eye alloc/scatter
### from the hot path.  (Benchmark-oriented buffer reuse; single-stream safe.)
_PHI1_M_CACHE: dict = {}


def _phi1_aug_M(B: int, N: int, device) -> torch.Tensor:
    key = (B, N, device)
    M = _PHI1_M_CACHE.get(key)
    if M is None:
        M = torch.zeros(B, 3 * N, 3 * N, dtype=torch.float32, device=device)
        M[:, N:2 * N, 2 * N:3 * N] = torch.eye(N, dtype=torch.float32, device=device)
        _PHI1_M_CACHE[key] = M
    return M
...
class _ExpmT18AugmentedTritonFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_E: torch.Tensor, grad_psi: torch.Tensor):
        ...
        ### = L_phi_1(A^T, G_psi).
        ### Cached pre-zeroed buffer: identity + zero blocks written once on
        ### allocation; only the three data blocks are rewritten here.
        M = _phi1_aug_M(B, N, A_T.device)
        M[:,  :N,      :N   ] = A_T
        M[:,  :N,     N:2*N ] = G_psi
        M[:, N:2*N,   N:2*N ] = A_T
