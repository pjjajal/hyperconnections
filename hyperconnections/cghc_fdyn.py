...
class ContinuousHyperConnectionsFeatDyn(nn.Module):
    def __init__(
        self,
        n: int,
        m: int,
    ...
    def compute_generator(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # --- Conservative branch ---
        if hasattr(self, "conserv_A"):
            M = self.conserv_A + self.conv_pred(x).reshape(B, self.n, self.n)
            logit_conserv = self.log_dt_conserv + self.dt_proj_conserv(x)  # [B, n]
        ...
        if (
            ...
            logit_diss = self.log_dt_diss + self.dt_proj_diss(x)  # [B, n]
        ...
        if hasattr(self, "diss_A"):
            R = self.diss_A + self.diss_pred(x).reshape(B, self.n, self.n)
        ...
        if hasattr(self, "diss_diag"):
            d = F.softplus(self.diss_diag + self.diss_pred(x))  # [B, n], positive
        ...
        if hasattr(self, "laplacian_A"):
            ...
            scores = self.laplacian_pred(x).reshape(B, self.n, self.n)
    ...
    def compute_read_write_weights(self, x: torch.Tensor):
        ...
        h_read_in = self.proj_read_in(x).reshape(B, self.n, self.m)
        h_write_out = self.proj_write_out(x).reshape(B, self.n, self.m)
